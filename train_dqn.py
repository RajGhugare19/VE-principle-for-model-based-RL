...
from models import DQN, ModelRL
...
replay_buffer = Memory(mem_size)
model = ModelRL(learning_rate,h_model,n_states,n_actions,rank_model).to(device)

if exp == 'VE':    
    VE_path =  'pretrained/VE/' + exp + str(rank_model) + "_" + str(value_width) + '.pt'
    model.load_state_dict(torch.load(VE_path))
    model.eval()
else:
    MLE_path = 'pretrained/MLE/' + exp + str(rank_model) + "_" + str(value_width) + '.pt'
    model.load_state_dict(torch.load(MLE_path))
    model.eval()

log = 'logs/dqn_' + exp + str(rank_model) + '_' + str(value_width)
    ...
    if n%target_update == 0:
     ...
     if len(scores) == 100:
        ...
        if avg > best_avg:
            #Saving best model
            best_avg = avg
            path = 'saved/dqn_logs/' + exp + str(rank_model) + "_" + str(value_width) + '.pt'
